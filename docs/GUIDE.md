...
Disable with `config.output.anti_hallucination_rules = false` (default: `true`).
...
All **22 built-in tools** are **read-only** and **idempotent** — they never modify your application or database. Hosts can append more via `config.additional_tools`.
...
Runs a single read-only SELECT statement on the app's established ActiveRecord connection and returns compact JSON.

**Parameters:**

| Param | Type | Description |
|-------|------|-------------|
| `sql` | string | **Required.** A single plain SELECT statement. Semicolons, CTEs (`WITH`), and mutating/locking clauses are rejected. |
...
Returns a redacted tail of a log file under the app's `log/` directory as compact JSON.
...
| `rubydex_incremental_threshold` | Float | `0.3` | Fraction of changed files (0.0–1.0) above which `IncrementalIndexer` performs a full rebuild instead of patching the existing graph |
| `rubydex_persist_index` | Boolean | `false` | Write the mtime snapshot to disk so incremental re-indexing survives process restarts |

**Incremental indexing:** `IncrementalIndexer` records file mtimes as integer seconds to avoid floating-point drift. On each subsequent call it diffs the current mtime map against the snapshot and only re-parses changed files. When `rubydex_persist_index` is `true`, the snapshot is saved alongside the index so the benefit extends across process restarts. A full rebuild is triggered automatically when the fraction of changed files exceeds `rubydex_incremental_threshold`.
...
RailsAiBridge.configure do |config|
  ...
  # Run introspectors concurrently using a fixed thread pool (requires concurrent-ruby).
  # Falls back to sequential when the AR connection pool has only one slot (e.g. transactional tests).
  # config.parallel_introspection    = true
  # config.parallel_pool_size        = 4   # upper bound; actual size = min(introspectors, pool_size)
  # config.parallel_timeout_seconds  = 10  # seconds before a future is cancelled and returns { error: }
