...
- **Endpoint policy and provider error hierarchy** (#178) — `Registry::EndpointPolicy` validates provider URLs against scheme (case-insensitive), host allowlist, loopback ports, and private-network rules. It resolves hosts through an injected resolver and returns a canonical URI plus an approved address set, failing closed with typed `ContextProviderError` / `PolicyError` results.
