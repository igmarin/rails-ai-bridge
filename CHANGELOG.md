...
- **Provider error message sanitization** — `ContextProviderClient` now redacts URLs (http, https, ssh, git, file, ftp, sftp, ws, wss), `git@` SSH URIs, and absolute file paths from error messages before constructing `ConnectionError` results. This prevents raw transport errors from leaking endpoints, query strings, or file paths into provider error results or Doctor output.
