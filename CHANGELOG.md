...
- **Endpoint policy and provider error hierarchy** (#178) — `Registry::EndpointPolicy` validates provider URLs against scheme, host allowlist, loopback ports, and private-network rules. It resolves hosts through an injected resolver and returns a canonical URI plus an approved address set, failing closed with typed `ContextProviderError` / `PolicyError` results.
- **ContextProviderClient** (#178) — `Registry::ContextProviderClient` validates the endpoint through `EndpointPolicy`, checks remote tool metadata for `read_only_hint` / `destructive_hint`, calls the tool through an injected transport factory, normalizes the result into `ContextProviderClient::Result`, and closes the transport in an `ensure` block. Errors are translated to `ContextProviderError` subclasses without leaking credentials.
