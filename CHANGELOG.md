...
- **ContextAggregator and ProviderRequestScope** (#179) — `Registry::ContextAggregator` orchestrates fetching from all declared context providers through `ContextProviderClient` instances. `fetch_all` iterates providers and their tools sequentially in deterministic order, maps results to declared fields (simple tool name or mapped field), enforces `max_providers`, `max_tools_per_provider`, and `aggregation_budget_seconds` caps, and returns an `AggregateResult` with `:success`, `:partial_failure`, or `:error` status. Optional provider failures are recorded as warnings and skipped; required provider failures are visible in the aggregate. `fetch_one` fetches a single provider by name. Mapping collisions are rejected before any network calls. `Registry::ProviderRequestScope` provides per-invocation memoization via `fetch_or_store` — it is not a process-wide cache. `ContextToolSpec#field_name` returns the context field name for simple or mapped tools.
