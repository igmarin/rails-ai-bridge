...
- **Endpoint policy and provider error hierarchy** (#178) — `Registry::EndpointPolicy` validates provider URLs against scheme (case-insensitive), host allowlist, loopback ports, and private-network rules. It rejects URLs containing credentials, restricts plain HTTP to loopback or allowed private destinations, and returns a canonical URI plus an approved address set, failing closed with typed `ContextProviderError` / `PolicyError` results. Parser and resolver errors are also translated to policy failures so exceptions never escape the client boundary.
- **ContextProviderClient** (#178) — `Registry::ContextProviderClient` validates the endpoint through `EndpointPolicy`, checks remote tool metadata for `read_only_hint` / `destructive_hint`, calls the tool through an injected transport factory, normalizes the result into `ContextProviderClient::Result`, and closes the transport in an `ensure` block. Auth resolver and transport failures return typed `ContextProviderError` results with sanitized, credential-free messages; close failures do not overwrite the original result.
