  ...
  module Tools
    ...
    class GetStimulus < BaseTool
      ...
      def self.call(controller: nil, detail: "standard", server_context: nil)
        ...
        if controller
          info = controllers.find { |entry| entry[:name].to_s.casecmp?(controller) }
          return text_response("Stimulus controller '#{controller}' not found.") unless info

          return text_response(format_controller(info))
        end

        case detail
        when "summary"
          text_response(format_summary(controllers))
        when "full"
          text_response(format_full(controllers))
        else
          text_response(format_standard(controllers))
        end
      end

      private_class_method def self.format_summary(controllers)
      ...
      private_class_method def self.format_standard(controllers)
        lines = [ "# Stimulus Controllers (#{controllers.size})", "" ]
        controllers.sort_by { |entry| entry[:name].to_s }.each do |entry|
          lines << "## #{entry[:name]}"
      ...
      private_class_method def self.format_full(controllers)
        lines = [ "# Stimulus Controllers (#{controllers.size})", "" ]
        controllers.sort_by { |entry| entry[:name].to_s }.each do |entry|
          lines << "## #{entry[:name]}"
      ...
      private_class_method def self.format_controller(entry)
        lines = [ "# #{entry[:name]}", "" ]
        lines << "- File: #{entry[:file]}" if entry[:file]
