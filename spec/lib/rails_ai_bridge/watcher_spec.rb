  ...
  describe '#start' do
    ...
    context 'when no watch directories exist' do
      before do
        ...
        allow(watcher).to receive(:require).with('listen').and_return(true)
      end
      ...
      it 'returns early without calling Listen.to' do
        ...
        begin
          ...
          allow(w).to receive(:require).and_call_original
          allow(w).to receive(:require).with('listen').and_return(true)
    ...
    context 'when watch directories exist' do
      ...
      it 'starts Listen on resolved roots and stops the listener on Interrupt' do
        allow(Listen).to receive(:to) do |*dirs, **_kwargs, &_block|
